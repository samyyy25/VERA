import { IncidentIntelligence, RiskLevel } from '../../types/complaint';

export interface IncidentIntelligenceInput {
  category: string;
  description: string;
  riskScore: number;
  riskLevel: RiskLevel;
 ...
 */
export function buildIncidentIntelligence(input: IncidentIntelligenceInput): IncidentIntelligence {
  const text = (input.description || '').toLowerCase();

  // 1. Standardized Incident Type
  let incidentType = input.category || 'Civic Issue';
  if (incidentType === 'Accident') incidentType = 'Road Accident';
  if (incidentType === 'Fire') incidentType = 'Fire Outbreak';
  if (incidentType === 'Medical Emergency') incidentType = 'Medical Emergency';
