...
export class AIService {
   ...
   */
  async isOmniRouteReachable(): Promise<boolean> {
    try {
      const res = await axios.get(`${config.omniRoute.baseUrl}/models`, {
        timeout: 2000,
   ...
   */
  async getNextEmergencyQuestion(
...
Initial citizen description: "${initialDescription}"`;
        ...
        const response = await axios.post(
          `${config.omniRoute.baseUrl}/chat/completions`,
          {
            model: config.omniRoute.model,
            messages,
            temperature: 0.3,
            max_tokens: 60,
          },
        ...
        if (aiText) {
          return {
            message: aiText,
            source: 'omniroute',
            modelUsed: config.omniRoute.model,
          };
        }
      } catch (err: any) {
   ...
   */
  async generateResponderSummary(
    category: string,
    description: string,
    conversationHistory: ChatMessage[],
    address?: string
