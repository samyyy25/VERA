...
import { DEMO_COMPLAINTS, DEMO_INCIDENTS, DEMO_EVENTS } from '../utils/demoSeed';
...
async function runSeed() {
  ...
  try {
    console.log(`📡 Seeding ${DEMO_COMPLAINTS.length} demo complaints into Supabase...`);
    for (const comp of DEMO_COMPLAINTS) {
      const payload = {
        id: comp.id,
    ...
    console.log(`📡 Seeding ${DEMO_INCIDENTS.length} escalated incidents into Supabase...`);
    ...
    console.log(`📡 Seeding ${DEMO_EVENTS.length} audit events into Supabase...`);
