...
async function testAllFourFeatures() {
  ...
  const roadDept = getDepartmentForCategory('Road damage');
  console.log('Road damage routed to:', roadDept.department, `[SLA: ${roadDept.responseSLA}]`);
  if (roadDept.department.includes('Roads Dept')) console.log('✅ Authority mapping for Road damage PASS');

  const accidentDept = getDepartmentForCategory('Accident');
  console.log('Accident routed to:', accidentDept.department, `[Emergency: ${accidentDept.isEmergencyService}]`);
  if (accidentDept.department.includes('Police Dept')) console.log('✅ Authority mapping for Accident PASS');
  ...
  const { events } = await complaintStore.getComplaintById(result.complaint.id);
  ...
  if (hasNotificationEvent) console.log('✅ Section 40: Notification logged to timeline PASS');
  if (hasJitsiEvent) console.log('✅ Section 41: Jitsi provisioning logged to timeline PASS');
