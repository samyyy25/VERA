...
exports.riskEngine = exports.RiskEngine = exports.calculateDistanceMeters = void 0;
...
class RiskEngine {
     ...
     */
    evaluateRisk(text, category, latitude, longitude, deviceSessionId, existingComplaints = []) {
        ...
        const recentReportsBySameDevice = existingComplaints.filter(c => c.device_session_id === deviceSessionId &&
        ...
        const nearbyReports = existingComplaints.filter(c => {
            if (new Date(c.created_at).getTime() < thirtyMinutesAgo)
                return false;
            const distance = (0, exports.calculateDistanceMeters)(latitude, longitude, c.latitude, c.longitude);
            return distance <= 100;
        });
