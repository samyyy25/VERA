...
exports.detectDuplicateIncident = detectDuplicateIncident;
 ...
 */
function detectDuplicateIncident(input) {
    ...
    for (const candidate of activeCandidates) {
        const distanceMeters = (0, riskScorer_1.calculateDistanceMeters)(input.latitude, input.longitude, candidate.latitude, candidate.longitude);
        // Distance threshold: <= 150 meters
        if (distanceMeters <= 150) {
            const timeDiffMinutes = Math.max(0, Math.round((now - new Date(candidate.created_at).getTime()) / (60 * 1000)));
            const textSim = calculateTextSimilarity(input.description, candidate.description);
            const sameCategory = input.category.toLowerCase().trim() === (candidate.category || '').toLowerCase().trim();
            ...
            const similarityScore = +(0.45 * proximityScore +
                0.25 * recencyScore +
                0.2 * catScore +
                0.1 * textSim).toFixed(2);
    ...
    if (bestMatch) {
        ...
        const clusterMembers = input.existingComplaints.filter(c => c.id === bestMatch.complaint.id ||
