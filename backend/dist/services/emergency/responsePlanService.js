...
exports.responsePlanService = exports.ResponsePlanService = exports.CATEGORY_AUTHORITY_MAP = void 0;
const overpassService_1 = require("./overpassService");
...
class ResponsePlanService {
    ...
    mapResponderToUnit(loc, defaultStatus = 'Alerted') {
     ...
     */
    generateFallbackResponders(category, latitude, longitude) {
        const config = exports.CATEGORY_AUTHORITY_MAP[category] || exports.CATEGORY_AUTHORITY_MAP['Other civic issues'];
        ...
        const hospitalDistKm = +((0, riskScorer_1.calculateDistanceMeters)(latitude, longitude, hospitalLat, hospitalLon) / 1000).toFixed(1);
        const policeDistKm = +((0, riskScorer_1.calculateDistanceMeters)(latitude, longitude, policeLat, policeLon) / 1000).toFixed(1);
        const municipalDistKm = +((0, riskScorer_1.calculateDistanceMeters)(latitude, longitude, municipalLat, municipalLon) / 1000).toFixed(1);
        const hospital = {
            ...
            directions_url: overpassService_1.overpassService.getDirectionsLink(latitude, longitude, hospitalLat, hospitalLon),
        ...
        const police = {
            ...
            directions_url: overpassService_1.overpassService.getDirectionsLink(latitude, longitude, policeLat, policeLon),
            latitude: policeLat,
            longitude: policeLon,
            status: 'Alerted',
        };
        const municipal = {
            ...
            directions_url: overpassService_1.overpassService.getDirectionsLink(latitude, longitude, municipalLat, municipalLon),
        ...
        const primaryMapped = {
            ...
            directions_url: overpassService_1.overpassService.getDirectionsLink(latitude, longitude, primaryLat, primaryLon),
            latitude: primaryLat,
        ...
        const secondaryMapped = {
            ...
            directions_url: overpassService_1.overpassService.getDirectionsLink(latitude, longitude, secondaryLat, secondaryLon),
     ...
     */
    async generateResponsePlan(complaint) {
        const lat = complaint.latitude;
        const lon = complaint.longitude;
        const category = complaint.category;
        const priority = complaint.risk_level || 'CRITICAL';
        const config = exports.CATEGORY_AUTHORITY_MAP[category] || exports.CATEGORY_AUTHORITY_MAP['Other civic issues'];
        // 1. Fetch nearby physical responders via Overpass / OSM
        const searchResult = await overpassService_1.overpassService.findNearbyResponders(lat, lon, 6000);
        const allResponders = [];
        ...
        let primaryResponse = fallbacks.primaryMapped;
        let secondaryResponse = fallbacks.secondaryMapped;
        ...
        if (!allResponders.some(r => r.type === 'municipal'))
            allResponders.push(fallbacks.municipal);
        return {
     ...
     */
    confirmPlan(plan, confirmedBy = 'AUTHORIZED_OPERATOR') {
        const now = new Date().toISOString();
        const updatedResponders = plan.all_responders.map(r => ({
            ...r,
            status: r.type === 'hospital' ? 'Alerted' : 'Dispatched',
        }));
        return {
            ...plan,
...
exports.responsePlanService = new ResponsePlanService();
